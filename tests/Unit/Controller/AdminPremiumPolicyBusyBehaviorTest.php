...
class AdminPremiumPolicyBusyBehaviorTest extends TestCase
{
	public function testUpdateNotificationSettingsReturns409WhenPremiumPolicyLockHeld(): void
	{
		...
		$appConfig->expects($this->atLeastOnce())->method('setAppValueString');
		$appConfig->method('setAppValueString')->willReturnCallback(
			static function (string $key, string $value): void {
				if ($key === Constants::CONFIG_PREMIUM_POLICY_JSON
					|| $key === Constants::CONFIG_PREMIUM_POLICY_VERSION) {
					throw new \RuntimeException('premium policy must not be written while lock busy');
				}
			}
		);
