	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		function resolveScopeAndApi(reportType, startDate, endDate) {
			...
			if (A.isAdmin) {
				...
				if (scope === 'admin_team') {
					const teamUsers = reportTeamUsersInput && reportTeamUsersInput.value ? reportTeamUsersInput.value : '';
					return {
			...
			if (A.isManager) {
				...
				if (scope === 'manager_single_team') {
					const managerTeamUsers =
						reportTeamUsersInput && reportTeamUsersInput.value ? reportTeamUsersInput.value : '';
					return {
						apiUrl: apiMap.team || null,
						isTeam: true,
						queryParams: {
		...
		function fetchAndShowReport() {
			...
			if (
				(reportScopeInput.value === 'admin_team' ||
					reportScopeInput.value === 'manager_single_team') &&
				(!reportTeamUsersInput || !reportTeamUsersInput.value)
			) {
				const teamMsg =
					(A.l10n && A.l10n.teamRequired) ||
			...
			fetch(url, { method: 'GET', headers: { requesttoken: requestToken } })
				...
				.then((result) => {
					...
					if (result.ok && data && data.success && data.report) {
						previewContent.innerHTML = `<p class="report-success">${esc(
							(A.l10n && A.l10n.reportReady) || 'Report generated successfully.',
						)}</p>${renderReportHtml(data.report)}`;
...
/**
 * Reports page behavior for ArbeitszeitCheck.
 *
 * Uses configuration from `window.ArbeitszeitCheck` that is bootstrapped
 * in the PHP template (page name, permissions, API URLs, translations).
 */
(() => {
	'use strict';

	function onReady() {
		// Ensure configuration and access permissions are present
		if (!window.ArbeitszeitCheck) {
			return;
		}

		const config = window.ArbeitszeitCheck;

		// Only run on the reports page and only for users who can access reports
		if (config.page !== 'reports' || !config.canAccessReports) {
			return;
		}
		...
		// React to changes in scope radios and team selects
		if (scopeForm) {
			scopeForm.addEventListener('change', (e) => {
				...
				// Enable/disable team selects based on active scope
				const scopeOrg = document.getElementById('scope-organization');
				const scopeAdminTeam = document.getElementById('scope-admin-team');
				const scopeManagerTeam = document.getElementById('scope-my-team');
				const scopeManagerSingleTeam = document.getElementById('scope-manager-single-team');
				const adminTeamSelect = document.getElementById('admin-team-select');
				const managerTeamSelect = document.getElementById('manager-team-select');
		...
		reportCards.forEach((card) => {
			card.addEventListener('click', (e) => {
				// Don't trigger if clicking the button
				if (e.target && e.target.classList.contains('btn-select-report')) {
					return;
				}
				const button = card.querySelector('.btn-select-report');
				if (button) {
		...
		function renderReportHtml(report) {
			...
			if (report.daily_breakdown && Object.keys(report.daily_breakdown).length) {
				html += `<h4 class="report-subhead">${L.dailyBreakdown || 'Daily breakdown'}</h4><table class="report-table"><thead><tr><th>${L.date || 'Date'}</th><th>${L.hours || 'Hours'}</th></tr></thead><tbody>`;
				Object.keys(report.daily_breakdown).sort().forEach((d) => {
					const day = report.daily_breakdown[d];
					html += `<tr><td>${esc(day.date || d)}</td><td>${esc(day.total_hours != null ? day.total_hours : '-')}</td></tr>`;
				});
				html += '</tbody></table>';
		...
		function resolveScopeAndApi(reportType, startDate, endDate) {
			...
			if (config.isManager) {
				...
				if (scope === 'manager_single_team') {
					const managerTeamUsers = (reportTeamUsersInput && reportTeamUsersInput.value) ? reportTeamUsersInput.value : '';
					return {
						apiUrl: apiMap.team || null,
						isTeam: true,
						queryParams: {
		...
		function fetchAndShowReport() {
			const reportType = reportTypeInput ? reportTypeInput.value : '';
			const dp = window.ArbeitszeitCheckDatepicker;
			const toISO = dp ? dp.convertEuropeanToISO : (s) => s;
			const startDate = toISO(startDateInput ? startDateInput.value : '');
			const endDate = toISO(endDateInput ? endDateInput.value : '');
			const previewSection = document.getElementById('report-preview');
			const previewContent = document.getElementById('report-preview-content');
			if (!previewSection || !previewContent) return;
			if (!reportType || !startDate || !endDate) {
				const errMsg = (config.l10n && config.l10n.error) || 'Please fill in report type, start date and end date.';
				announceToScreenReader(errMsg);
				previewContent.innerHTML = `<p class="report-error" role="alert">${esc(errMsg)}</p>`;
				previewSection.style.display = 'block';
				previewSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
			...
			updateScopeFromForm();
			if (!reportScopeInput || !reportScopeInput.value) {
				const scopeMsg = (config.l10n && config.l10n.scopeRequired) || 'Please choose who should be included in the report.';
				announceToScreenReader(scopeMsg);
				previewContent.innerHTML = `<p class="report-error" role="alert">${esc(scopeMsg)}</p>`;
				previewSection.style.display = 'block';
				previewSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
			...
			const scopeResolution = resolveScopeAndApi(reportType, startDate, endDate);
			if (!scopeResolution.apiUrl) {
				const typeMsg = (config.l10n && config.l10n.error) || 'Invalid report type.';
				previewContent.innerHTML = `<p class="report-error" role="alert">${esc(typeMsg)}</p>`;
				previewSection.style.display = 'block';
				previewSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
				return;
			...
			// If a team-specific scope is selected, ensure a team or user set is present
			if (
				(reportScopeInput.value === 'admin_team' || reportScopeInput.value === 'manager_single_team') &&
				(!reportTeamUsersInput || !reportTeamUsersInput.value)
			) {
				const teamMsg = (config.l10n && config.l10n.teamRequired) || 'Please select a team.';
				announceToScreenReader(teamMsg);
				previewContent.innerHTML = `<p class="report-error" role="alert">${esc(teamMsg)}</p>`;
				previewSection.style.display = 'block';
				previewSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
			...
			if (previewBtn) {
				previewBtn.disabled = true;
				previewBtn.textContent = (config.l10n && config.l10n.generating) || 'Generating...';
			}
			if (generateBtn) {
				generateBtn.disabled = true;
				generateBtn.textContent = (config.l10n && config.l10n.generating) || 'Generating...';
			}
			previewContent.innerHTML = `<p class="report-loading" aria-busy="true">${esc(
				(config.l10n && config.l10n.generating) || 'Generating report...',
			)}</p>`;
			previewSection.style.display = 'block';
			announceToScreenReader((config.l10n && config.l10n.generating) || 'Generating report...');
				...
				.then((result) => {
					...
					if (result.ok && data && data.success && data.report) {
						previewContent.innerHTML =
							`<p class="report-success">${esc(
								(config.l10n && config.l10n.reportReady) || 'Report generated successfully.',
							)}</p>` + renderReportHtml(data.report);
						announceToScreenReader(
							(config.l10n && config.l10n.reportReady) || 'Report generated successfully.',
						);
						...
						const msg =
							(data && data.error) ||
							(result.status === 403 || result.status === 401
								? config.l10n.sessionExpired ||
								  'Your session may have expired. Please refresh the page and try again.'
								: config.l10n.error || 'An error occurred');
						previewContent.innerHTML = `<p class="report-error" role="alert">${esc(msg)}</p>`;
						announceToScreenReader(msg);
						previewSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
					}
		...
		function downloadReport() {
			...
			const exportBase =
				config.exportUrl && config.exportUrl[exportKey]
					? config.exportUrl[exportKey]
					: null;
			if (!exportBase) {
				return;
			}
