...
class Version1003Date20250103000000 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		...
		if ($schema->hasTable('at_entries')) {
			...
			if (!$table->hasIndex('at_entries_user_start_idx')) {
				$table->addIndex(['user_id', 'start_time'], 'at_entries_user_start_idx');
				$changed = true;
			}
			// Add composite index for user_id + status (for active entry queries)
			if (!$table->hasIndex('at_entries_user_status_idx')) {
				$table->addIndex(['user_id', 'status'], 'at_entries_user_status_idx');
				$changed = true;
			}
		}
		...
		if ($schema->hasTable('at_absences')) {
			...
			if (!$table->hasIndex('at_absences_user_status_idx')) {
				$table->addIndex(['user_id', 'status'], 'at_absences_user_status_idx');
				$changed = true;
			}
		}
		...
		if ($schema->hasTable('at_violations')) {
			...
			if (!$table->hasIndex('at_violations_user_resolved_idx')) {
				$table->addIndex(['user_id', 'resolved'], 'at_violations_user_resolved_idx');
				$changed = true;
			}
			// Add composite index for user_id + date (for date range queries)
			if (!$table->hasIndex('at_violations_user_date_idx')) {
				$table->addIndex(['user_id', 'date'], 'at_violations_user_date_idx');
				$changed = true;
			}
		}
		...
		if ($schema->hasTable('at_audit')) {
			...
			if (!$table->hasIndex('at_audit_user_created_idx')) {
				$table->addIndex(['user_id', 'created_at'], 'at_audit_user_created_idx');
				$changed = true;
			}
			// Add composite index for entity_type + entity_id (for entity-specific queries)
			if (!$table->hasIndex('at_audit_entity_idx')) {
				$table->addIndex(['entity_type', 'entity_id'], 'at_audit_entity_idx');
				$changed = true;
			}
		}
